<script lang="ts">
	...
	// Options untuk dropdown - pastikan nilai sesuai dengan database
	...
	const penggajianOptions = [
		{ value: 'harian', label: 'Harian' },
		{ value: 'bulanan', label: 'Bulanan' }
	];

	// Pilihan shift kerja - dari database master_data dengan format yang informatif
	$: shiftKerjaOptions = data?.masterData?.shift?.map(shift => ({
		value: shift.value,
		label: shift.jam_masuk && shift.jam_keluar 
			? `${shift.label} : ${shift.jam_masuk} - ${shift.jam_keluar}`
			: shift.label
	}))

	// Debug log untuk shift options
	$: if (shiftKerjaOptions) {
		console.log('🔍 Debug shiftKerjaOptions:', shiftKerjaOptions);
	}
	...
	$: if (form?.values && form?.errors) {
		...
		jabatan = form.values.jabatan || jabatan;
		penggajian = form.values.penggajian || penggajian;
		shift = form.values.shift || shift;
	...
	// Reactive statement untuk handle success
	$: if (form?.success && !form?.errors) {
		...
		// Navigate to employees page after success
		setTimeout(() => {
			goto('/employees');
		}, 1500); // Delay 1.5 detik untuk menampilkan notifikasi
	}
	...
	// Di bagian script, tambahkan reactive statement untuk handle errors
	$: if (form?.errors) {
		// Pastikan form tidak freeze saat ada error
		isLoading = false;
		console.log('🚨 Form validation errors detected:', form.errors);

		// Show error notification
		showNotification(
			'error',
			'Terdapat kesalahan dalam pengisian form. Silakan periksa field yang ditandai.'
		);
	}
	...
	// Reactive statement untuk populate form fields dari data yang diterima
	$: if (data?.employee) {
		nama_lengkap = data.employee.nama_lengkap || '';
		no_karyawan = data.employee.no_karyawan || '';
		tanggal_masuk = data.employee.tanggal_masuk || '';
		status_kerja = data.employee.status_kerja || '';
		divisi = data.employee.divisi || '';
		jabatan = data.employee.jabatan || '';
		penggajian = data.employee.penggajian || '';
		shift = data.employee.shift || '';
		no_telp = data.employee.no_telp || '';
		email = data.employee.email || '';
		kontak_darurat = data.employee.kontak_darurat || '';
		nip = data.employee.nip || '';
		no_ktp = data.employee.no_ktp || '';
		no_kk = data.employee.no_kk || '';
		no_npwp = data.employee.no_npwp || '';
		tempat_tanggal_lahir = data.employee.tempat_tanggal_lahir || '';
		alamat_ktp = data.employee.alamat_ktp || '';
		alamat_domisili = data.employee.alamat_domisili || '';
		asal_kota = data.employee.asal_kota || '';
		umur = data.employee.umur || '';
		status_hubungan = data.employee.status_hubungan || '';
		agama = data.employee.agama || '';
		tanggungan = data.employee.tanggungan || '';
		pendidikan_terakhir = data.employee.pendidikan_terakhir || '';
		no_bpjs = data.employee.no_bpjs || '';
		faskes_tingkat_1 = data.employee.faskes_tingkat_1 || '';
		nama_bank = data.employee.nama_bank || '';
		no_rekening_bank = data.employee.no_rekening_bank || '';
		kelamin = data.employee.kelamin || '';
		lokasi_absen = data.employee.lokasi_absen || '';
	}
</script>
	...
	<div class="border-b border-gray-200 bg-white px-6 py-4">
		...
		<div class="flex items-center justify-between">
			<div>
				...
				<p class="mt-1 text-gray-600">Ubah data karyawan yang sudah terdaftar dalam sistem</p>
			</div>
		...
		<div class="p-6">
			<div class="rounded-lg bg-white p-6 shadow-sm">
				<div class="flex items-center justify-center py-12">
					<div class="flex items-center">
						<svg
							class="mr-3 -ml-1 h-8 w-8 animate-spin text-blue-600"
							xmlns="http://www.w3.org/2000/svg"
							fill="none"
							viewBox="0 0 24 24"
						>
		...
		<div class="p-6">
				...
				use:enhance={() => {
					...
					return async ({ result, update }) => {
						...
						if (result.type === 'failure') {
							// Form validation error - keep form editable
							console.log('Form validation failed:', result.data?.errors);
							await update(); // Update form with errors
						} else if (result.type === 'success') {
							// Success - show notification
							console.log('Form submitted successfully');
							await update();
						} else {
							// Other cases - still update
							await update();
						}
					};
				}}
			...
			>
				...
				<div class="rounded-lg bg-white p-6 shadow-sm">
					...
					<div
						class="rounded-lg border-2 border-dashed border-gray-300 bg-white p-6 text-center transition-all duration-200 hover:border-blue-400 hover:bg-blue-50"
					>
						<div class="flex flex-col items-center">
							...
							<p class="mb-2 text-sm font-medium text-gray-600">Upload foto KTP karyawan</p>
							<p class="mb-3 text-xs text-gray-400">Format: JPG, PNG, PDF (Max 5MB)</p>
							<input
								type="file"
								name="foto_ktp"
								id="foto_ktp"
								accept=".png, .jpg, .jpeg, .gif, .image, .webp, .svg, .pdf"
								on:change={handleFileUpload}
								class="hidden"
							/>
							<label
								for="foto_ktp"
								class="cursor-pointer rounded-lg bg-blue-600 px-6 py-2 text-sm text-white shadow-sm transition-colors duration-200 hover:bg-blue-700"
							>
					...
					<div class="grid grid-cols-1 gap-x-12 gap-y-6 lg:grid-cols-2">
						...
						<div class="space-y-6">
							...
							<div>
								...
								>
									<option value="">Pilih Sistem Penggajian</option>
									{#each penggajianOptions as option}
										<option value={option.value}>{option.label}</option>
									{/each}
								</select>
							...
							<div>
								...
								<select
									...
									bind:value={shift}
									class="w-full rounded-lg border border-gray-300 px-4 py-3 transition-all duration-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500"
									class:border-red-500={form?.errors?.shift}
								>
									<option value="">Pilih Shift</option>
									{#each shiftKerjaOptions as option}
										<option value={option.value}>{option.label}</option>
									{/each}
								</select>
								{#if form?.errors?.shift}
									<p class="mt-1 text-sm text-red-600">{form.errors.shift}</p>
								{/if}
							</div>
							...
							<!-- 14. Umur -->
							<div>
								<label for="umur" class="mb-2 flex items-center text-sm font-medium text-gray-700">
									<svg
										class="mr-2 h-4 w-4 text-gray-500"
										fill="none"
										stroke="currentColor"
						...
						<div class="space-y-6">
							...
							<div>
								...
								<input
									...
									placeholder="Contoh: K/0, K/1, K/2"
									class="w-full rounded-lg border border-gray-300 px-4 py-3 transition-all duration-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500"
							...
							<!-- 19. NIP -->
							<div>
								<label for="nip" class="mb-2 flex items-center text-sm font-medium text-gray-700">
									<svg
										class="mr-2 h-4 w-4 text-gray-500"
										fill="none"
										stroke="currentColor"
							...
							<!-- 21. No KK -->
							<div>
								<label for="no_kk" class="mb-2 flex items-center text-sm font-medium text-gray-700">
									<svg
										class="mr-2 h-4 w-4 text-gray-500"
										fill="none"
										stroke="currentColor"
