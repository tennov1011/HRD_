...
export async function load({ cookies }) {
	...
	if (session) {
		try {
			...
			console.log('Employee page - User session:', {
				email: user.email,
				role: user.role,
				divisi: user.divisi
			});
	...
	try {
		...
		if (response.ok) {
			...
			if (user) {
				...
				console.log(`🔍 Division filtering: ${originalCount} -> ${employees.length} employees`);

				// Debug division mapping if needed
				if (user.role === 'manager_divisi') {
					debugDivisionMapping(data.data || [], user);
				}
			}
