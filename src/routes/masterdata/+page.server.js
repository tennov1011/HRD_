...
export async function load() {
	try {
		...
		const groupedData = {
			...
			lokasi_absen: allData.filter(
				/** @param {any} item */ (item) => item.category === 'lokasi_absen'
			),
...
export const actions = {
	...
	// Action untuk mengupdate divisi
	updateDivisi: async ({ request }) => {
		...
		if (!id || !nama || !status) {
			return fail(400, { error: 'ID, nama divisi, dan status wajib diisi' });
		}

		try {
			await api.patch(`/items/master_data/${id}`, {
				nama: nama,
				status: status
			});
			...
			return fail(500, { error: 'Gagal mengupdate divisi' });
		}
	},

	// Action untuk mengupdate jabatan
	updateJabatan: async ({ request }) => {
		const data = await request.formData();
		const id = data.get('id');
		const nama = data.get('nama');
		const status = data.get('status');
		...
		if (!id || !nama || !status) {
			return fail(400, { error: 'ID, nama jabatan, dan status wajib diisi' });
		}
		...
		try {
			...
			return fail(500, { error: 'Gagal mengupdate jabatan' });
		}
	},

	// Action untuk mengupdate lokasi absen
	updateLokasiAbsen: async ({ request }) => {
		const data = await request.formData();
		const id = data.get('id');
		const nama = data.get('nama');
		const alamat = data.get('alamat');
		...
		if (!id || !nama || !status) {
			return fail(400, { error: 'ID, nama lokasi, dan status wajib diisi' });
		}
		...
		try {
			...
			console.error('Error updating lokasi absen:', error);
			return fail(500, { error: 'Gagal mengupdate lokasi absen' });
		}
	},

	// Action untuk mengupdate shift
	updateShift: async ({ request }) => {
		...
		if (!id || !nama || !jam_masuk || !jam_keluar) {
			return fail(400, { error: 'ID, nama shift, jam masuk, dan jam keluar wajib diisi' });
		}
		...
		try {
			...
			await api.patch(`/items/master_data/${id}`, payload);
			...
			return fail(500, { error: `Gagal mengupdate shift: ${errorMessage}` });
		}
	}
};
