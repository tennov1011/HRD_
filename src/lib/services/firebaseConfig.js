...
// Create stores for authentication state with proper typing
...
export const roleDefinitions = [
	// Updated login configurations as requested
	{ email: 'hrd@eltama.com', role: 'admin', department: 'HRD', name: 'HRD Admin' },
	{ email: 'manager.hrd@eltama.com', role: 'manager', department: 'HRD', name: 'Manager HRD' },
	{
	...
	// Manager Divisi roles - First level approval for their division
	{ email: 'manager.it@eltama.com', role: 'manager_divisi', department: 'IT', name: 'Manager IT' },
	{
...
// Store to track if stores are manually set (from server-side auth)
let storesManuallySet = false;

// Function to manually set stores (called from Header.svelte)
export function setUserStores(userData) {
	console.log('🔧 Firebase: Setting stores manually:', userData);
	storesManuallySet = true;
	isAuthenticated.set(userData.authenticated || false);
	// @ts-ignore - Allow string values in stores
	userRole.set(userData.role || null);
	// @ts-ignore - Allow string values in stores
	userEmail.set(userData.email || null);
	// @ts-ignore - Allow string values in stores
	userName.set(userData.name || null);
	// @ts-ignore - Allow string values in stores
	userDepartment.set(userData.department || null);
}

// Function to clear stores (called during logout)
export function clearUserStores() {
	console.log('🔧 Firebase: Clearing stores manually');
	storesManuallySet = false;
...
// Function to set user info from session (used by server-side auth)
export function setUserFromSession(sessionData) {
	if (browser && sessionData) {
		console.log('Setting user from session:', sessionData);
		storesManuallySet = true;

		isAuthenticated.set(true);
		userRole.set(sessionData.role || 'user');
		userDepartment.set(sessionData.divisi || '');
		userName.set(sessionData.nama || '');
		userEmail.set(sessionData.email || '');
	}
}
...
if (browser && auth) {
	onAuthStateChanged(auth, (user) => {
		// If stores are manually set (server-side auth), don't override them
		if (storesManuallySet) {
			console.log('🔧 Firebase: Stores manually set, skipping Firebase auth override');
			return;
		}
		...
		if (user && user.email) {
			...
			const userDefinition = roleDefinitions.find(
				(def) => def.email.toLowerCase() === user.email.toLowerCase()
			);
	...
	// If not in browser or auth not available, set initial state (only if not manually set)
	if (!storesManuallySet) {
...
export function isManagerRole(role) {
	return (
		role && (role.endsWith('.manager') || role === 'general_manager' || role === 'plant_manager')
	);
}
...
export function canUserApprove(userRole, userDepartment, procurement) {
	...
	if (currentStage === 'department_manager') {
		...
		console.log(
			`Department manager check: isManager=${isManager}, isDepartmentMatch=${isDepartmentMatch}`
		);
...
export function checkDepartmentAccess(userDepartment, procurementDepartment) {
	...
	if (userDeptUpper.includes(',')) {
		...
		console.log(
			`Multi-department check: ${departments.join(', ')} includes ${procDeptUpper}? ${hasAccess}`
		);
		return hasAccess;
	}

	// Special case: multi_department.manager role has access to multiple departments
	const isMultiDeptManager = roleDefinitions.some(
		(def) =>
			def.email.toLowerCase() ===
				(auth && auth.currentUser ? auth.currentUser.email.toLowerCase() : '') &&
