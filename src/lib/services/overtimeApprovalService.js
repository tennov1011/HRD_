// Service untuk Multi-Level Overtime Approval System
...
/**
 * Approval stages configuration - sama seperti leave approval
...
/**
 * Get current user approval permissions - sama seperti leave approval
 */
export function getCurrentUserApprovalLevel() {
	...
	console.log('🔍 DEBUG getCurrentUserApprovalLevel (Overtime):', {
		role,
		email,
...
export function canApproveAtStage(request, stage) {
	...
	console.log('🔍 DEBUG canApproveAtStage (Overtime):', {
	...
	if (
		...
		if (isFromSameDivision) {
			canApprove = true;
			console.log('🔍 DEBUG: MANAGER DIVISI CAN APPROVE OVERTIME! 🎯', {
				userLevel,
				requestStage: request.approval_stage,
				requestEmployeeEmail: request.employee_email || request.email,
		...
		canApprove = true;
		console.log('🔍 DEBUG: HRD ADMIN CAN APPROVE OVERTIME! 🎯', {
			userLevel,
			requestStage: request.approval_stage,
		...
		canApprove = true;
		console.log('🔍 DEBUG: MANAGER HRD CAN APPROVE OVERTIME! 🎯', {
			userLevel,
			requestStage: request.approval_stage,
		...
		canApprove = true;
		console.log('🔍 DEBUG: DIREKTUR CAN APPROVE OVERTIME! 🎯', {
			userLevel,
			requestStage: request.approval_stage,
	...
	if (!canApprove) {
		console.log('❌ DEBUG: Overtime Approval DENIED - Reason:', {
			userLevel,
			requestStage: request.approval_stage,
			explanation: `User level "${userLevel}" cannot approve overtime request at stage "${request.approval_stage}"`
		});
	}

	console.log('🔍 DEBUG canApprove result (Overtime):', canApprove);
	return canApprove;
}

/**
 * Check if overtime request is from same division as current manager
 */
function checkIfRequestFromSameDivision(request, managerEmail) {
	...
	console.log('🔍 DEBUG checkIfRequestFromSameDivision (Overtime):', {
		managerEmail,
		managerDivision,
		employeeDivision,
		requestEmployeeEmail: request.employee_email || request.email,
...
function getCurrentUserName() {
	...
	console.log('🔍 DEBUG getCurrentUserName (Overtime):', {
		email,
		role,
		storedName: name,
		mappedName,
...
/**
 * Approve overtime request at current stage
 */
export async function approveOvertimeAtStage(request, stage, reason = '') {
	...
	console.log('🔍 DEBUG approveOvertimeAtStage - Current User Info:', {
	...
	// Update database - untuk overtime request
	try {
		const response = await directusApi(`items/lembur/${request.id}`, {
			method: 'PATCH',
		...
		return {
			...
			message: `Pengajuan lembur telah disetujui di level ${stage}`
		};
	} catch (error) {
...
/**
 * Reject overtime request at current stage
 */
export async function rejectOvertimeAtStage(request, stage, reason = '') {
	const currentUser = getCurrentUserName();
	const currentDate = new Date().toISOString().split('T')[0];

	// 🔧 DEBUG: Log current user performing rejection
	console.log('🔍 DEBUG rejectOvertimeAtStage - Current User Info:', {
		userName: get(userName),
		userEmail: get(userEmail),
		userRole: get(userRole),
		currentUser: currentUser,
	...
	try {
		...
		return {
			success: true,
			data: response.data,
			message: `Pengajuan lembur telah ditolak di level ${stage}`
		};
	} catch (error) {
...
export function getAvailableActions(request) {
	...
	console.log('🔍 DEBUG getAvailableActions (Overtime):', {
		requestId: request.id,
		userLevel,
		currentStage,
	...
	console.log('🔍 DEBUG final actions (Overtime):', actions);
	return actions;
}
